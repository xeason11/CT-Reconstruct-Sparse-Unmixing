'''
cite by https://github.com/preetam22n/DeepTrans-HSU
...
class Train_test:
    def __init__(self, dataset, device, skip_train=False, save=False):
        ...
        if dataset == 'HongHe_roisquare':
            ...
            self.LR, self.EPOCH = 0.001, 400#改
            self.patch, self.dim = 2, 400
            self.beta, self.gamma = 5e3, 1e-4#改
            self.weight_decay_param = 3e-5#改
            # self.order_abd, self.order_endmem = (3, 1, 2, 0), (3, 1, 2, 0) 不用调，已经是水、林、沼泽湿地、建设了
            ...
            self.LR, self.EPOCH = 0.001, 200  # 改
    ...
    def run(self, smry):
        ...
        if not self.skip_train:
            ...
            m=int((time_end - time_start)/60)
            s=int(time_end - time_start-m*60)
            print('Total computational time cost:{} m {} s'.format(m,s))
        ...
        data_l = sio.loadmat('data/try_linear3.mat')
        plots.plot_abundance(target, abu_est,data_l['Y_hat'],self.P, self.save_dir)
